...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    const ROLE_HOME: Record<string, string> = {
      SUPER_ADMIN: "/pages/super-admin", MODERATOR: "/pages/moderator",
      REGULAR_DALAAL: "/pages/broker", VERIFIED_DALAAL: "/pages/broker",
      PROPERTY_OWNER: "/pages/owner", VEHICLE_OWNER: "/pages/owner", CUSTOMER: "/pages/customer",
    };
      ...
      <div className="min-h-screen flex flex-col bg-zinc-50 dark:bg-zinc-950 font-sans text-zinc-950 dark:text-zinc-50">
        ...
        <main className="flex-1 flex items-center justify-center px-4 py-16">
          <div className="text-center max-w-md">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-8">Your email has been verified. You can now access all features.</p>
            <button onClick={() => {
              const ROLE_HOME: Record<string, string> = {
                SUPER_ADMIN: "/pages/super-admin", MODERATOR: "/pages/moderator",
                REGULAR_DALAAL: "/pages/broker", VERIFIED_DALAAL: "/pages/broker",
        ...
        <div className="relative z-10 w-full max-w-md">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-sky-200 bg-sky-50 dark:border-sky-900/50 dark:bg-sky-950/30 text-[11px] font-bold text-sky-700 dark:text-sky-300 mb-5">
              <Shield className="w-3 h-3" /><span>Verify Your Email</span>
            </div>
            <h1 className="text-3xl font-black tracking-tight">Check Your Inbox</h1>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-2">
              {email ? <>Enter the 6-digit code sent to <strong>{email}</strong></> : "Enter your email to verify your account"}
            </p>
          </div>
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/60 rounded-2xl p-7 shadow-xl">
            {error && (
              <div className="mb-4 p-3 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 text-xs font-semibold text-red-600">{error}</div>
            )}
            <form onSubmit={handleVerify} className="space-y-5">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-zinc-600 dark:text-zinc-400 block">Verification Code</label>
                <input type="text" placeholder="123456" maxLength={6} value={code} onChange={(e) => { setCode(e.target.value.replace(/\D/g, "")); setError(""); }}
              ...
              <button type="submit" disabled={isLoading || code.length !== 6}
                ...
                {isLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><span>Verify Email</span><ArrowRight className="w-4 h-4" /></>}
              </button>
            </form>
