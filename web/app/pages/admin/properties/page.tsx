...
export default function AdminProperties() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await adminService.getAllListings({ type: "PROPERTY", limit: "100" });
  ...
  const handleDelete = async (id: string) => {
    try {
      setDeleting(true);
      await adminService.getAllListings ? fetch("/api/admin/listings/" + id, { method: "DELETE" }) : null;
    ...
    <div className="flex flex-1 flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {listings.length} total propert{listings.length !== 1 ? "ies" : "y"} across
            all brokers
          </p>
        </div>
        <div className="flex items-center gap-2 bg-muted/50 border border-border px-4 py-2 rounded-[10px] text-sm font-semibold text-muted-foreground">
          <Building2 className="w-4 h-4" /> {listings.length} Total
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <h3 className="font-bold text-lg mb-1">Failed to load properties</h3>
        ...
        <div className="text-center py-16">
          ...
          <h3 className="font-bold text-lg mb-1">
            {listings.length === 0 ? "No properties yet" : "No results found"}
          </h3>
          <p className="text-sm text-muted-foreground">
            {listings.length === 0
              ? "Properties will appear here once brokers create listings."
