...
export default function AdminCustomers() {
  ...
  useEffect(() => {
    async function fetchUsers() {
      try {
        const data = await adminService.getUsers({ role: "CUSTOMER", limit: "50" });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage registered customers on Dalaal
          </p>
        </div>
      </div>
                    ...
                    filtered.map((u) => (
                      <tr key={u.id} className="hover:bg-muted/20 transition-colors">
                        ...
                        <td className="py-3 px-4 text-center">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border bg-orange-100 text-orange-700 dark:bg-orange-950/40 dark:text-orange-400 border-orange-200 dark:border-orange-900">
                            {u.role.replace("_", " ")}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center">
