...
export default function EVCPlusPage() {
  ...
  useEffect(() => {
    async function fetchEVC() {
      try {
        const result = await api.get("/admin/payments?provider=EVC_PLUS");
        const list = Array.isArray(result) ? result : result?.payments ?? result?.data ?? [];
        setTransactions(list);
      } catch (err: any) {
        setError(err.message || "Failed to load EVC Plus transactions");
      } finally {
        setLoading(false);
      }
    }
      ...
      <div className="rounded-xl border bg-white dark:bg-zinc-950 shadow-sm overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {filtered.length === 0 ? (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-muted-foreground">No EVC Plus transactions found</td></tr>
            ) : (
              filtered.map((t: any) => {
                const user = t.user?.profile?.firstName ? `${t.user.profile.firstName} ${t.user.profile.lastName || ""}`.trim() : t.user?.username || t.user?.email || "—";
                return (
